...
const ModalDespesasVariaveis = ({ idDespesa, Aberto, Fechado }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userData = await api.get(`http://localhost:5000/despesa_var/${idDespesa}`, {
          body: {
            userId: userID,
            despesaVariavelId: idDespesa,
          },
          headers: {
  ...
  async function handleEnvio(event) {
    ...
    try {
      const response = await api.patch(`http://localhost:5000/despesa_var/${idDespesa}`, body, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (response.data.status === true) {
        toast.success("Despesa Variável alterada com sucesso!");
        Fechado();
