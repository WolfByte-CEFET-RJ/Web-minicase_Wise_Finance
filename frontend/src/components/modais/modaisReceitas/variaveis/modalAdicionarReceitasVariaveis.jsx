...
const ModalReceitasVariaveis = ({ Aberto, Fechado }) => {
  ...
  async function handleEnvio(event) {
    ...
    try {
      const response = await api.post("http://localhost:5000/receita_var", body, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (response.data.status === true) {
        toast.success("Receita Variável adicionada com sucesso!");
        Fechado();
      ...
      <div className="w-[70%] h-[100%]  absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white  rounded-[96px]">
        <div className="container-modal">
          <img
            ...
            onClick={Fechado}
          />
          <h1 className=" mt-[-3.6%] ml-[26%] text-[50px] font-black text-green">
            Receitas Variaveis
          </h1>
